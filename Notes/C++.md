...
​	本小节参考自[C++多线程——读写锁shared_lock/shared_mutex](https://blog.csdn.net/princeteng/article/details/103952888)。

​	读写锁也叫做**共享-独占锁**。当读写锁以**读模式**锁住时，它是以**共享模式**锁住的；当它以写模式锁住时，它是以独占模式锁住的。

​	1）当读写锁处于**写加锁**状态时，在其解锁之前，**所有尝试对其加锁的线程都会被阻塞**；

​	2）当读写锁处于**读加锁**状态时，所有**试图以读模式对其加锁的线程都可以得到访问权**，但是如果**想以写模式对其加锁，线程将阻塞**。

#### 4.1) std::shared_mutex(C++17)

​	shared_mutex拥有二个访问级别：

- 共享：多个线程能共享同一互斥的所有权；
- 独占性：仅一个线程能占有互斥。

1）若一个线程已经通过lock或try_lock获取**独占锁(写锁)**，则其他线程无法获取该锁(**包括共享的**)。尝试获得锁的线程会被阻塞。
2）仅当任何线程均**未获取独占性锁时**，**共享锁(读锁)**才能被多个线程获取(lock_shared、try_lock_shared)。
3）在一个线程内，同一时刻只能获取一个锁(共享或独占性)。

| 排他性锁定        |                                                              |
| ----------------- | ------------------------------------------------------------ |
| `lock`            | 锁定互斥量。<br>若另一线程已锁定互斥量，则lock的调用线程将阻塞执行，直至获得锁。<br>已经以读模式锁/写模式获得锁的线程再次调用lock的话，会产生未定义行为。 |
| `try_lock`        | 尝试锁定互斥量，若互斥量不可用则返回。                       |
| `unlock`          | 解锁互斥量，如果当前线程不拥有该互斥量仍调用unlock，会产生未定义行为。<br>通常用std::unique_lock和std::lock_guard管理排他锁。 |
| **共享性锁定**    |                                                              |
| `lock_shared`     | 获得互斥量的共享所有权。<br>若另一线程以**排他性所有权保**持有互斥量，则lock_shared的调用者将阻塞，直到能取得共享所有权。<br>若以读模式/写模式**拥有锁**的线程再次调用lock_shared，会产生**未定义行为**，可能**产生死锁**。 |
| `try_lock_shared` | 尝试共享所有权锁定互斥量                                     |
| `unlock_shared`   | 将互斥量从调用线程的共享所有权释放。<br>当前执行线程必须以共享模式锁定互斥量，否则行为未定义。通常使用**std::shared_lock**管理。 |
