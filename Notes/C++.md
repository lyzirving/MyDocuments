...
​	constexpr还有很多用法，下面只调了部分进行说明，参考自[这里](https://blog.csdn.net/janeqi1987/article/details/103542802)。
...
​	map内部实现是**红黑树**。红黑树具有**自动排序**的功能，因此map内部所有元素都是有序的。
...
​	vector的通过一个**连续的数组**存放元素。

​	如果集合已满，在新增数据的时候，就要分配一块更大的内存，将原来的数据**复制**过来，**释放之前的内存**，再插入新增的元素。

### 2) vector存放指针的问题

​	vector扩容时会复制原数据，删除原内存，申请新内存。

​	因此，如果有指针指向此前的地址，释放后，指针将不能正常访问到此数据。
