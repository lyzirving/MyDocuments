...
Canvas组件，即画布。把各类UI元素放到Canvas下后，Canvas的工作就是合并这些元素。
...
- Canvas下UI元素的合并规则
  ...
  同一个Canvas中，将**相同层级**(Depth)和**相同材质**的元素进行合并。
...
- 图元设置更新`Graphics.SetAllDirty()`
  ...
  `SetVerticesDirty`、`SetMaterialDirty`会调用`CanvasUpdateRegistry.RegisterCanvasElementForGraphicRebuild`。将需要重构的元件数据加入IndexedSet容器中，等待下次重构。

  `CanvasUpdateRegistry`只负责收集网格，不负责渲染和合并。

- 注册待更新图元`CanvasUpdateRegistry.RegisterCanvasElementForGraphicRebuild`
...
- 更新图元`CanvasUpdateRegistry.PerformUpdate`
...
#### 1) Body Transform && Root Transform
