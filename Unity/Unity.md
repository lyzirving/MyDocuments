...
- 排序规则：物体以何种顺序渲染；
- 剔除规则：应该绘制哪些物体；
- 资源管理：整个过程中存在哪些缓冲区(颜色、深度、法线、运动矢量等)；
- 流程控制：何时绘制阴影，何时运行后处理，通道(pass)如何被调度以及中间结果如何在它们之间流动。

### 三种类型的渲染管线

- 固定功能管线(无可编程着色器)：渲染流程完全由**硬件**预先固化，是一个不可更改的、线性的操作序列。开发者无法编写自定义的着色程序，只能调整已经存在的东西，但无法注入新的、每像素级别的算法。

- 框架固定，着色器可编程的渲染管线：主流游戏引擎(以Unity内置管线为代表)的经典架构模式。
...
<img src="/pic_vertex_shader.png" alt="pic_vertex_shader" style="zoom:80%;" />

- **Position Shading**：对每个顶点进行变换，将顶点位置从模型空间变换到裁剪空间。
- **Face Test Culling**：面剔除。根据三角形的朝向(顺时针或逆时针)决定是否剔除。
- **Frustum Test Culling**：视锥体剔除。将不在视锥体内的图元剔除。
- **Sample Test Culling**：采样测试剔除，通常称为“小图元剔除”或“子像素剔除”，剔除小到连1个像素都没有的多边形。
- **Varying Shading**：顶点着色器中输出的其他变量(如颜色、纹理坐标等)的插值计算。在光栅化过程中，这些变量会被插值到每个片段。
- **Polygon List生成多边形列表**：生成多边形列表，为后续光栅化阶准备。这里会将**Tile List写入到显存中**，发生一次**内存写入**。

在**标准的图形管线**中，逐顶点执行的顶点着色器之后还有可选的几何着色器、曲面细分等阶段，然后进行图元组装。

之后才执行如面剔除、视锥体剔、采样剔除除等操作。

#### 3 片元着色器

下述是Arm架构，若想深入了解，可参考：[ARM Mobile Studio性能优化](https://developer.unity.cn/projects/5f7935c3edbc2a001f898bff)。

<img src="/pic_fragment_pass.png" alt="pic_fragment_pass" style="zoom:50%;" />

- GPU从内存中取出**Tile List**，发生一次内存读取。

- 开始光栅化，将多边形变成2D。

- 开始进行Early Z Test。知道深度的话，就可以提前将被挡住的像素删掉。但shader中使用了Alpha Test的Discard Clip这种操作，此时GPU就无法相信光栅化后的排序。

- 创建**Fragment线程**，开始执行片元着色器。

- Late Z，剔除被遮挡的像素。但此时Fragment的压力是很大的，所以最好优先被Early Z剔除。

- Blender，开始混合上色。

- Tile RAM & Tile Write将颜色写入到每个Tile中。此时还在GPU中，**不会占用带宽**。

- Transaction Elimination。假如你在游戏中不动，前后2帧有大面积的像素不需要刷新，所以GPU会给每个Tile中的所有颜色计算一个**类似Hash的东西**和现在的Frame Buffer做比较。

  颜色发生改变，才将该Tile写入到显存中，从而进一步节省带宽。

### SRP(待完善)

- SRP渲染管线的基类是RenderPipeline.cs，它抽象了Render方法：
  ...
  //
  // Parameters:
  //   context: 整个SRP渲染的最底层接口, 常用接口如下:
  //            Cull()
  //            DrawRenderers()渲染多个对象
  //            DrawShadows()渲染阴影
  //            ExecuteCommandBuffer()
  //            DrawSkybox()
  //            Submit()正式提交渲染命令
  //
  //   cameras: 相机堆栈, 存在Base Camera和Overlay Camera, 需要根据它们的深度以及关系来进行排序
  //            Game窗口通常会出现相机叠加(3D摄像机 + UI摄像机)
  protected abstract void Render(ScriptableRenderContext context, Camera[] cameras);
  ```

- Base Camera + Overlay Camera：

  在UniverdsalRenderPipeline.cs中的`RenderCameraStack`中，URP先绘制**Base Camera**，再通过for循环渲染每一个**Overlay Camera**。其中通过`RenderSingleCamera`，对每个相机进行真正的渲染。

- 正向渲染器UniversalRenderer

  其内部内置了很多Pass，每个Pass需要指定自己的RenderPassEvent，决定执行顺序：

  <img src="/pic_forward_renderer.png" alt="pic_forward_renderer" style="zoom:80%;" />
...
- ScriptableRenderPass
  ...
  自定义Pass需要继承ScriptableRenderPass，最常用的两个Pass是绘制不透明Pass和半透明Pass：

  ```c#
  m_RenderOpaqueForwardPass = new DrawObjectsPass(URPProfileId.DrawOpaqueObjects, true, RenderPassEvent.BeforeRenderingOpaques, RenderQueueRange.opaque, data.opaqueLayerMask, m_DefaultStencilState, stencilData.stencilReference);x
...
本小节参考自：[带宽：GPU 的粮道在哪里堵住了（发烫优化系列 · 第 2 篇）](https://zhuanlan.zhihu.com/p/2078469144383580153)。

手机发烫，烧的不是"算力"，而是"搬运"——Arm 官方给出的数字是 **DRAM 访问每 GB/s 要付 80–100mW 的电**。

| 概念              | 说的是什么       | 类比                 | 常见单位 |
| ----------------- | ---------------- | -------------------- | -------- |
| 内存容量          | 能存多少数据     | 仓库面积             | GB       |
| 带宽（Bandwidth） | 每秒能搬多少数据 | 马路每小时能过多少车 | GB/s     |
| 延迟（Latency）   | 一次往返要多久   | 单程耗时             | ns       |

手机主流的 LPDDR5 带宽大约在 25–60 GB/s 这个量级，而 PC 独立显卡的 GDDR6 显存能到几百 GB/s。

**手机天生比 PC 窄一个数量级**，而你的游戏要跑的画面，复杂度可没有低一个数量级。

### 为什么"搬运"这么贵：存储层级金字塔

现代芯片的存储是一个金字塔。金字塔越往上越快越小，越往下越大越慢。

关键在于：**缓存（SRAM）和内存（DRAM）是两种完全不同的物理器件**：

| DRAM                         | SRAM                        |                          |
| ---------------------------- | --------------------------- | ------------------------ |
| Dynamic Random Access Memory | Static Random Access Memory |                          |
| 存一个 bit                   | 1 个电容 + 1 个晶体管       | 6 个晶体管               |
| 速度                         | 约 100ns                    | 几 ns（快几十倍）        |
| 容量/成本                    | 便宜，能做几十 GB           | 贵，只能做几 KB～几十 MB |
| 用途                         | 内存（手机 LPDDR）          | CPU/GPU 的缓存           |

注意 DRAM 的一个特点：**它靠电容存电荷记 bit，电容会漏电，所以每隔几十毫秒必须把所有数据读一遍再写回去**——这就是"Dynamic（动态）"的含义。

也就是说，**DRAM 即使没人访问它，光维持数据就在耗电**。

所以 GPU 干活的真实流程是：先问缓存要数据，缓存里有（命中）就便宜快速；缓存里没有（未命中）就得去 DRAM 搬——**跨芯片跑一个来回，延迟上百倍，功耗几十倍**。

这就是那句"一次内存访问的耗电，够 ALU 算几十次"的来历。
...
GPU片内的 SRAM（比如 TBDR 的 tile memory、GPU 缓存）带宽非常高，但容量很小；

DRAM 容量大，但带宽有限且访问功耗高。

所以瓶颈通常不在 GPU 内部，而在**GPU 与 DRAM 之间的通道**。
...
GPU 内存控制器
      ↕
GDDR6 / GDDR6X / HBM 显存（VRAM）
      ↕
GPU 内部缓存（L1 / L2 / 共享内存）
      ↕
GPU 着色器核心 / 纹理单元 / 光栅化单元

上述链路可概述为：**系统内存 ↔ PCIe ↔ 显存 ↔ GPU 内部缓存 ↔ GPU 核心**。

- **CPU 和 GPU 各有自己的内存控制器**，分别管理系统内存和显存。
- **两者之间唯一的通道是 PCIe 总线**。
- **GPU 渲染时，数据主要在 VRAM ↔ GPU 内部之间流动，不走 PCIe。**

因此，瓶颈也要分情况：

- 纯GPU渲染：当分辨率高、纹理大、后处理复杂时，显存带宽先吃满，GPU 核心等数据，帧率下降。

  瓶颈在 **GPU ↔ VRAM** 这一段。

- CPU和GPU频繁交互：每帧大量传入动态纹理、顶点；从GPU回读结果；资源流式加载。
  此时瓶颈在 **PCIe 总线**。

### 移动 GPU 的自救：[TBDR](https://zhida.zhihu.com/search?content_id=282715166&content_type=Article&match_order=1&q=TBDR&zhida_source=entity) 架构

移动端 GPU（Mali、Adreno）在设计上和 PC GPU 走了完全不同的路线——**TBDR（Tile-Based Deferred [Rendering](https://zhida.zhihu.com/search?content_id=282715166&content_type=Article&match_order=1&q=Rendering&zhida_source=entity)，基于分块的渲染）**：

1. 把画面切成一块块小 tile（比如 32×32 像素）；
2. 先对整帧做几何处理，算出每个 tile 里有哪些图元；
   - 几何处理：GPU 对场景中所有物体的顶点做计算，计算每个图元在屏幕上的位置。主要包括：**顶点着色器**、**图元装配**、**裁剪**、**透视除法 & 视口变换**、**背面剔除、视锥剔除**等。
   - 对于每个三角形，判断它覆盖了哪些 tile。
   - 把三角形的索引，加入到这些 tile 的 **图元列表（tile list）** 中。
3. 对每个 tile，在GPU芯片内部一块很小的 SRAM（tile memory）里完成所有着色和混合；
4. 算完才把最终结果写回 DRAM。

PC 的"立即模式渲染"——画什么写什么，帧缓冲读写全走显存。

TBDR 相当于**"攒一批货，用小推车在车间里倒腾完，最后只跑一趟大仓库"**。

所以某些在 PC 上无害的写法在手机上格外致命——比如频繁切换渲染目标、大量乱序小绘制，都会破坏 tile 的批处理，让"本该一趟的搬运"变成多趟。

### 带宽被谁吃掉了

- **纹理采样（最大头）**——片元着色器每输出一个像素基本都要读纹理。纹理越大、每像素采样次数越多（多重采样、法线贴图、[光照贴图](https://zhida.zhihu.com/search?content_id=282715166&content_type=Article&match_order=1&q=光照贴图&zhida_source=entity)……），搬运越猛。
- **帧缓冲读写**——每个透明像素 = 读旧色 + 混合 + 写新色。Overdraw 叠 N 层，这里就乘 N。这是上一篇"搬运惯犯"Overdraw 的账单明细。
- **全屏后处理**——每个 pass 都把整屏像素读写一遍。Bloom + 景深 + Color Grading 挂三个，整屏数据就多跑六个来回（读+写各算一次）。

### 节省带宽的几个方向

| 方向       | 手段                                                         | 原理                         |
| ---------- | ------------------------------------------------------------ | ---------------------------- |
| 少搬       | 纹理压缩（[ASTC](https://zhida.zhihu.com/search?content_id=282715166&content_type=Article&match_order=1&q=ASTC&zhida_source=entity)） | 每像素字节数降到 1/9         |
| 搬得巧     | Mipmap                                                       | 数据在缓存里挨得近、命中率高 |
| 别重复搬   | 减 Overdraw                                                  | 帧缓冲读写 ×N → ×1           |
| 少生成像素 | 降 Render Scale                                              | 像素数平方级下降             |
...
为什么这招好用？因为它只精准打击"随像素数线性增长的搬运"，其他瓶颈不随分辨率变化——**这是一个天然的隔离实验**。

顺带一提，它反过来也告诉你一件事：如果你是带宽瓶颈，那"降 Render Scale 到 0.75"本身就是最便宜的救命药（像素量降 44%），比改任何代码都快。

## 内存&&垃圾回收GC

GC 是托管堆上回收不再被引用的对象，它会造成**主线程停顿(STW，Stop-The-World)**，产生卡顿尖刺、引发掉帧。

Mono 下支持增量 GC，能把一部分回收工作分摊到多帧；

IL2CPP 用的是 Boehm GC，目前是完整的 STW 停顿，尖刺更明显。

所以需要注意：

- 少用会装箱的操作，比如老版本的foreach、把值类型塞进 object；
- 避免频繁 new字符串，拼接优先使用 [StringBuilder](https://zhida.zhihu.com/search?content_id=283156541&content_type=Article&match_order=1&q=StringBuilder&zhida_source=entity)，高频协程留意每帧 new 产生分配；
- 定位用 Profiler 的 GC Alloc 那一栏，看每帧托管内存分配了多少。
...
Draw Call 是 CPU 通知 GPU 绘制一批几何体的命令，它本身不是问题，**多了才会加重 CPU 提交开销**。

- 其中材质切换带来的 **SetPass Call 上涨**往往比 DrawCall 本身更影响性能，很多人会混淆这两个。
- 静态物件开启静态合批；
- 大量重复物件用 GPU Instancing；
- URP 管线善用 SRP Batcher 降低 CPU 提交开销；
- UI 依靠 Canvas 合批；图集打包减少纹理切换打断合批；
...
**Job System 是多线程任务调度器**，让工作安全并行；

**Burst 是编译器优化器**，将C#代码转为极致优化的机器码。

两者结合实现**多核并行+指令级优化**，性能提升10-100倍，但**只能用于纯计算任务**，不能访问Unity引擎对象。
...
- 空闲线程从忙碌线程的任务队列“偷”工作；
- 避免线程饥饿，确保所有CPU核心高效利用；
- 无锁调度，减少线程等待开销。

#### 2 安全内存访问

- Job是struct，无引用，避免堆分配；
- Native容器，非托管内存，明确所有权，不存在多线程竞争问题。
...
```c#
C#Job代码 → 优化中间码 → LLVM IR → 机器码
   ↓           ↓           ↓        ↓
托管.NET     Burst前端    LLVM优化  原生执行
   ↓           ↓           ↓        ↓
JIT编译    静态分析     平台优化   直接CPU
(慢)      (类型检查)   (SIMD)    指令
```
...
### JobSystem和Burst协同工作流程

```c#
1. 主线程准备数据
   ↓
2. 创建NativeContainer, 分配非托管内存
   ↓
3. 创建 Job 结构体（栈分配）
   ↓
4. Burst 编译 Job 代码 → 优化机器码
   ↓
5. Job System 调度 → 多线程执行
   ↓
6. 自动依赖管理 → 并行+安全
   ↓
7. 主线程等待结果 → 数据同步
...
- Model：纯粹的数据和业务逻辑。不依赖于View或ViewModel，通常包含数据模型、业务规则、数据访问等。
- View：用户界面，负责显示数据(从ViewModel获取)并将用户输入传递给ViewModel。在Unity中，View通常是MonoBehaviour。
- ViewModel：作为View和Model之间的桥梁。它暴露View需要的数据和命令，并将View的操作传递给Model。ViewModel通常实现INotifyPropertyChanged接口来通知View数据变化，从而实现数据绑定。
- 数据流向
  - 用户操作 → View 捕获 → ViewModel 处理 → Model 更新
  - 数据变化 → Model 通知 → ViewModel 转换 → View 刷新
- 关键优势
  - 解耦：改UI不动逻辑，改逻辑不动UI；
  - 复用：同一ViewModel可适配不同View；
  - 清晰：各司其职，代码结构一目了然。
- 痛点
  - Unity无原生MVVM支持，需要自己实现绑定系统
  - 预制体实例化复杂

### MVC和MVVM的区别

- 耦合性：Controller直接操作View和Model，而ViewModel通过数据绑定与View同步。
- 数据绑定：MVC中通常没有自动数据绑定，需要手动更新View；MVVM中View和ViewModel通过数据绑定自动同步。
- 依赖关系：在MVC中，View和Model都可能被Controller直接操作；在MVVM中，View只依赖ViewModel，Model也只被ViewModel操作。
